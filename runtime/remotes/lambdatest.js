...
function getCredentials() {
  ...
  if (program.aces) {
    cpPath = '../../../runtime/scripts/secrets/lambdatest.json';
  } else {
    cpPath = '../../runtime/scripts/secrets/lambdatest.json';
  }
  const secrets = loadConfig(cpPath);

  const user = process.env.LT_USERNAME || secrets.LAMBDATEST_USERNAME;
  const key = process.env.LT_ACCESS_KEY || secrets.LAMBDATEST_ACCESS_KEY;
